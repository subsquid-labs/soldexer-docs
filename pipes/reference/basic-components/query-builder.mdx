...
class SolanaQueryBuilder<F extends FieldSelection = {}> {
  ...
  addInstructionRequest(options: RequestOptions<InstructionRequest>): this
  addTransactionRequest(options: RequestOptions<TransactionRequest>): this
  addLogRequest(options: RequestOptions<LogRequest>): this
  addBalanceRequest(options: RequestOptions<BalanceRequest>): this
  addTokenBalanceRequest(options: RequestOptions<TokenBalanceRequest>): this
  addRewardRequest(options: RequestOptions<RewardRequest>): this
...
```ts
solanaQuery().addInstructionRequest({
  range: { from: 200_000_000 },
  request: {
    programId: ['whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc'],
...
## `.addTokenBalanceRequest(options)`
...
solanaPortalStream({
  ...
  outputs: { swaps: solanaQuery().addInstructionRequest({/*…*/}).build() },
})
```
