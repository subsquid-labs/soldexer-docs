...
sidebarTitle: "Production Pipe Example"
---
...
The entrypoint is [`pipes/evm/liquidity/cli.ts`](https://github.com/subsquid-labs/pipes-sqdgn-dex-example/blob/master/pipes/evm/liquidity/cli.ts). The pipeline is: **Portal stream → composite decoders → transform pipe → ClickHouse target**.
...
The CLI merges all decoders into a named `outputs` record (`outputs: { ...decoders }`) so the Portal stream is decoded into multiple typed streams (one per protocol).
...
Data flows from the Portal (with cache and optional metrics) through the named decoder outputs, then through the liquidity transform, and finally into ClickHouse. For run instructions and env vars, see the repo [README](https://github.com/subsquid-labs/pipes-sqdgn-dex-example#readme).
