...
The `pipeTo(createTarget({write, resolveFork}))` pattern keeps fork handling completely separate from batch processing. The SDK catches the 409 internally and calls `resolveFork()` with the portal's consensus slot sample; `write()` never sees the interruption and continues iterating batches without restarting.
...
<Step title="Implement resolveFork()">

`resolveFork()` receives `canonicalBlocks` — the portal's view of the canonical chain (named `previousBlocks` in the raw 409 payload) — and must return the last good slot cursor, or `null` if recovery is impossible:
...
async function main() {
  ...
  await solanaPortalStream({
    ...
    .pipeTo(createTarget({
      ...
      resolveFork: async (newConsensusSlots) => {
...
function pipeToIterator<T>(
  ...
  onFork: (canonicalBlocks: BlockCursor[]) => Promise<BlockCursor | null>
...
**The finalized slot as the last-resort anchor.** Keep the current finalized slot *in* your rollback history even though it is technically not unfinalized. It is the guaranteed safe floor: the portal will never ask you to roll back past it. Having it available means `resolveFork()` can always return a valid cursor for the deepest possible reorg. Pruning with `number > finalized` instead of `number >= finalized` removes this anchor and makes very deep reorgs unrecoverable.
...
**The cursor returned from `resolveFork()` is inclusive.** Return the last slot you consider good; the SDK resumes from `cursor.number + 1`. Off-by-one errors cause either duplicate re-processing or skipped slots.
...
**Forks only occur in the real-time (unfinalized) portion of the stream.** Finalized data does not reorganize: a 409 from the `/finalized-stream` endpoint only means the cursor's `parentBlockHash` is stale, for example carried over from an orphaned unfinalized block. Fork handling is only needed when consuming the `/stream` endpoint with `fromBlock` near or at the chain head. If your range is bounded and entirely in the past, you will never see a fork.
