...
<Note>
If your pipeline includes a [stateful transformer](../advanced-topics/stateful-transforms#fork-callbacks-and-crash-recovery), it must also implement a `fork` callback to roll back its own state in lockstep with the target.
</Note>
