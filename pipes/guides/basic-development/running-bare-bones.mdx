...
description: "Run a Solana Pipes SDK pipeline as a plain async iterator."
---

By default, `solanaPortalStream` activates a console logger. Passing `metrics` and `progress` enables those services. To embed a pipe into external code with no side effects, disable them all:
