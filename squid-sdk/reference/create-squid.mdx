...
* If a S3 URL is used, then the S3 region, endpoint and user credentials will be [taken from the default environment variables](sdk/reference/store/file/s3-dest/). Fill your `.env` file and/or set your [SQD Cloud secrets](/en/cloud/resources/env-variables/) accordingly.
...
```bash
curl --compress -o commands.json https://raw.githubusercontent.com/subsquid-labs/file-store-parquet-example/main/commands.json
```
